      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button @click="editor(scope.row)" type="text" size="small">完成</el-button>
...
export default {
  ...
  created: function() {
    ...
    that.total = null
    that.tableData = null
    const url = this.$url + '/api/order/queryUnfinished?id='
    ...
    editor(row) {
      ...
      axios.post(url, {orderID: row.orderID, detailID: row.repairmentID}).then(function (resp) {
        ...
        if (resp.data.code === 1000) {
          axios.get(that.$url + '/api/order/query?id=' + row.orderID).then(function (resp1) {
            axios.post(that.$url + '/api/comm/setCommStatus', {commissionID: resp1.data.commissionId, status: resp1.data.status}).then(function (resp2) {
              that.$message({
                showClose: true,
                message: '操作成功',
                type: 'success'
